...
<html>
   ...
   <?php
      ...
      $select_month_control = ' <form class="form-inline"  role="form"><div class="form-inline"><select class="form-control" style="width:132px;" name="month" id="month">';
      for($x = 1; $x <= 12; $x++) {
      ...
      $cont='<form method="get">'.$previous_month_link.'<tab>'.$next_month_link.' </form>';
      ...
      $events = array();
        if($month < 10) {
      	...
      	while($row = mysqli_fetch_assoc($result1)) {$arr= array();
      		$id=$row['id'];
